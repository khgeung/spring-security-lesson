...
/**
 ...
 역할 : Spring Filter Proxy 사용자 인증(로그인 체크) 시 DB와 연동해 인증 여부를 담당
 */
...
public class CustomMemberDetailsService implements UserDetailsService {
     ...
     * 2. Spring Security Filter Proxy 가 username 으로 아래 메소드 호출
     * 3. Repository를 이용해 사용자 정보 조회 후 userDetails (사용자 정보)를 반환
     * 4. Spring Security Filter Proxy가 입력한 비밀 번호와 UserDetails 비밀번호를 비교
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        //주의점 : findById 가 아니고 findByUsername 메소드를 호출
       Member findMember = memberRepository.findByUsername(username).orElseThrow(()->{
           log.error("인증 실패 - 존재하지 않는 사용자 : {}", username);
            return new UsernameNotFoundException("아이디에 해당하는 회원이 존재하지 않습니다" + username);
       });
       log.info("사용자 정보 확인 username={}, name={}, role={}", findMember.getUsername(), findMember.getName());
       findMember.getRole();
